...
if (process.env.DATABASE_URL) {
  ...
  // According to Supabase docs: Direct connection uses db.[PROJECT-REF].supabase.co with username 'postgres'
  // URL encode password if it contains special characters
  const password = encodeURIComponent(process.env.SUPABASE_DB_PASSWORD)
  connectionString = `postgresql://postgres:${password}@db.${supabaseUrl}.supabase.co:5432/postgres`
} else {
